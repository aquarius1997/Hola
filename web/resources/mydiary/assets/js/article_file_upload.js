...
// 첨부파일 drag & drop 이벤트 처리 : 파일업로드처리 -> 파일 출력
...
function uploadFile(formData) {
    $.ajax({
        ...
        data: formData,
        dataType: "text",
        processData: false, //데이터를 일반적인 query string으로 변환처리할 것인지 결정. (false:자동 변환 처리안함)
        contentType: false,
        type: "POST",
...
function printFiles(data) {
    ...
    // hnaldebars 파일 템플릿 컴파일을 통해 생성된 HTML을 DOM에 주입
...
function getFileInfo(fullName) {
    ...
    var imgSrc;             // 썸네일이나 파일아이콘 이미지 파일 출력요청 URL
